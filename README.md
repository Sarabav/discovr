...
│   ├── error.html         # Generic error page, rendered by the global errorhandler and billing failures
...
- **Env vars**: `SECRET_KEY` is auto-generated by Render. `FLASK_DEBUG` is
  ...
  `OPENROUTER_API_KEY`, `STRIPE_SECRET_KEY`, and the `SUPABASE_*` vars
  are marked `sync: false` — Render prompts for these in the dashboard
  rather than committing them; fill in `OPENROUTER_API_KEY` and
  `STRIPE_SECRET_KEY` at minimum (the chatbot and its paywall both
  need them), and the Supabase ones only if you plan to use [Supabase
  mode](#data-source-local-sqlite-or-supabase). `sync: false` only
  actually prompts you on a Blueprint sync — if the service was created
  by hand before `render.yaml` existed, adding a var to this file later
  doesn't retroactively add it to that service's dashboard. Check
  Settings → Environment directly if something that reads one of these
  starts failing after a config change here.

**Startup safety nets, so a bad or missing config degrades instead of
crashing every request:**
- If the (locally-persisted) Data Source is set to Supabase, `app.py`
  calls `ensure_schema()` before serving — if that fails (e.g.
  `SUPABASE_DB_URL` isn't set), it logs why and falls back to local
  SQLite rather than serving every subsequent query against a backend
  that was never actually set up.
- A global `@app.errorhandler(Exception)` catches anything no route
  handled itself, logs the full traceback to stderr, and renders a
  plain `error.html` instead of Flask's bare default 500 page. Normal
  HTTP errors (404, the 403 from `admin_required`, etc.) pass through
  untouched.
...
- **Stripe failures never reach the browser as a 500**: every Stripe
  ...
  address on your account."). `_client()` itself also catches a missing
  `STRIPE_SECRET_KEY` the same way — a real incident: the var was added
  to `render.yaml` as `sync: false` (Render prompts for it rather than
  storing it) but never actually set in an existing manually-created
  Render service, so every unpaid user's first authenticated page hit
  an unhandled `KeyError` reading it. `app.py` catches only
  `BillingError` — it never imports the `stripe` SDK itself — and
  renders `error.html` with that message directly (not a flash +
  redirect to `/dashboard`, which would just bounce a still-unpaid user
  straight back into the same failing `/checkout` — an infinite loop
