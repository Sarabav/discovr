...
- 💳 **`src/billing.py`** - Stripe: `create_checkout_session`/`confirm_checkout_session`/`refund_payment`; the only module that touches the `stripe` SDK, `STRIPE_SECRET_KEY` set lazily inside `_client()` (a missing key there raises `BillingError` too, not a raw `KeyError` — real incident, see README's Billing section); every call wraps `stripe.error.StripeError` and re-raises `BillingError` (plain message, real error logged) so `app.py` never sees a raw Stripe exception, and renders it directly rather than flash+redirect-to-`/dashboard` (would loop, since an unpaid user just bounces back into the same failing `/checkout`); `src.auth.is_valid_email` (real format check) is checked before an email ever reaches Stripe, both at signup and again at `/checkout`
