...
- 💳 Stripe (`src/billing.py`) — freemium, not a hard paywall: audits/findings free, generated fixes (except one free taster) + fix-all + audit re-runs are a one-time $5 via Checkout Sessions + refunds

## 🗂️ Code Structure
- 🚪 **`app.py`** - Flask App entry point; a global `@app.errorhandler(Exception)` logs the traceback and renders `error.html` instead of a bare 500 for anything a route didn't catch itself; at startup, falls back Data Source to `local` (logging why) if it's set to `supabase` but `ensure_schema()` fails
- ☁️ **`render.yaml`** - Render Blueprint: build/start commands, env vars, for `gunicorn app:app` deploys
- 🖼️ **`templates/`** - front end. `landing.html` is the public page at `/`; `dashboard.html` (chat-first, at `/dashboard`) is the whole logged-in app, reachable by every logged-in user regardless of payment (freemium — see `src/billing.py`) — includes a Settings button/panel showing payment status, a "Refund my payment" button for paid users or an Upgrade link for free ones; `header.html` is a shared partial included by every logged-in page (normal users see Dashboard/Settings only, plus an Admin link in the avatar dropdown if `is_admin`); `admin.html` (`/admin`, admin-only) hubs the developer views — Components, Workflows, Chunks, Results — each still its own page/route, just gated and reachable only from here, plus the Data Source dropdown (local/Supabase), "Clone Local Data to Supabase" button, and a Users table with a Refund button per paid user; `admin_bar.html` is the muted "Admin — internal tools" partial included on all admin pages; `workflows.html` (`/workflows`) lists the app's end-to-end workflows as run-able cards
...
- 💳 **`src/billing.py`** - Stripe: `create_checkout_session`/`confirm_checkout_session`/`refund_payment`; the only module that touches the `stripe` SDK, `STRIPE_SECRET_KEY` set lazily inside `_client()` (a missing key there raises `BillingError` too, not a raw `KeyError` — real incident, see README's Billing section); every call wraps `stripe.error.StripeError` and re-raises `BillingError` (plain message, real error logged) so `app.py` never sees a raw Stripe exception, and renders `error.html` directly rather than a flash+redirect; `src.auth.is_valid_email` (real format check) is checked before an email ever reaches Stripe, both at signup and again at `/checkout`. `app._taster_finding_id`/`app._upgrade_response` implement the freemium gate (see README's Billing section) — checked fresh per request on `/findings/<id>/fix`, `/chat`'s `run_audit`/`fix_all`/`generate_fix`/`check_progress` intents, never a page-only gate
